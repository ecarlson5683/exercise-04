...
play_wordle_KB <- function(solution, valid_list, num_guesses=6){
  ...
  num_guesses <- 6
  print(paste("Hi! You have", num_guesses, "chances to guess the", word_length, "lettered word")) # Numbers of guesses left
  
  # Display what letters the player has not yet guessed 
...
# Prompt a player for a guess
  ...
  guess <- readline(paste("Enter guess number", num_guesses, "then press <enter>:")) #Readline prompts user to enter a guess
  guess <- toupper(guess)
...
#      guess <- readline(paste("Invalid entry! Guess must have", word_length, " characters"))
#    }
  
 # guess_vector <- str_split(guess, "")
  
play_wordle_KB(solution, valid_list, num_guesses)
