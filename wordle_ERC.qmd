...
solution <- pick_solution(solution_list) # Run function to create a vector of single-character elements, the letters of the solution word
...
play_wordle <- function(solution, valid_list, num_guesses=6){
  n_length <- lengths(solution) # Return the length of every element in the list **solution**
  rules <- c("You", "have", num_guesses, "chances", "to", "guess", "a", "word", "of", "length", n_length) %>% # Create a vector of strings for rules sentence
    paste(., collapse = " ") %>% # Convert the vector of strings into a single string
    print() 
  
  alphabet <- LETTERS %>% toupper() %>% paste(., collapse = " ") # Return single string of alphabet
  letters_left <- paste0("Letters left: ", alphabet) %>% 
    print()

  guess <- paste0("Enter guess number ", num, " then press <enter>: ") # Error: should be guesses taken, not num_guesses; should convert guess to uppercase
  guess <- readline(guess) # Take a character string as an argument and provide a "prompt" entering a line of numeric or character data
  
  case_when(
    nchar(guess, type = "chars") != 5 ~ readline("Error: Not a 5-letter word. Re-enter guess then press <enter>: "), # Error: should loop back to guess
    !(guess %in% valid_list) ~ readline("Error: Not a valid word. Re-enter guess then press <enter>: "), # Error: should indicate guess number
    guess %in% valid_list & nchar(guess, type = "chars") == 5 ~ letters_left # Error: should go to evaluate_guess()
    ) 
